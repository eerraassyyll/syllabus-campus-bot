...
def save_user_setting(chat_id, key, value):
    ...
    if c.rowcount == 0:
        ...
        c.execute('INSERT INTO users (chat_id, role, user_id, selected_day) VALUES (?, ?, ?, ?)', 
                  (chat_id, value if key == 'role' else None, 
...
def get_user_settings(chat_id):
    conn = sqlite3.connect('bot.db')
    c = conn.cursor()
    c.execute('SELECT role, user_id, selected_day FROM users WHERE chat_id = ?', (chat_id,))
    row = c.fetchone()
    conn.close()
    if row:
...
def get_teachers_keyboard(data, page=0, per_page=27):
    ...
    buttons = [types.InlineKeyboardButton(text=t.get('_short', '???'), callback_data=f"set_teach_{t['_id']}") for t in page_teachers]
    markup.add(*buttons)
    
    # Навигация
    nav_row = []
    if page > 0:
        nav_row.append(types.InlineKeyboardButton("⬅️ Назад", callback_data=f"teach_page_{page-1}"))
    if end < len(teachers):
        nav_row.append(types.InlineKeyboardButton("Вперед ➡️", callback_data=f"teach_page_{page+1}"))
    if nav_row:
        markup.row(*nav_row)
...
def get_role_keyboard():
    ...
    markup.add(types.InlineKeyboardButton("👨‍🎓 Студент", callback_data="set_role_student"))
    markup.add(types.InlineKeyboardButton("👨‍🏫 Преподаватель", callback_data="set_role_teacher"))
    return markup

def get_main_menu():
    markup = types.InlineKeyboardMarkup()
    # Исправлено здесь и во всех остальных кнопках
    markup.add(types.InlineKeyboardButton("📅 Расписание", callback_data="get_schedule"))
    markup.add(types.InlineKeyboardButton("🚪 Свободные кабинеты", callback_data="get_free_rooms"))
...
def get_days_selection_keyboard():
    ...
    # Кнопка возврата
    markup.add(types.InlineKeyboardButton("🔙 Вернуться назад", callback_data="get_schedule"))
    return markup
...
def show_schedule_view(message, day_index):
    ...
    day_cards = [c for c in data.get('cards', {}).get('card', []) if c['_days'][day_index] == '1']
    ...
    for card in day_cards:
        ...
        if is_match:
            ...
            period_info = periods.get(period, {}).get('_starttime', '?')+" – "+periods.get(period, {}).get('_endtime', '?')
            ...
            if role == 'student':
                ...
                t_names = [teachers.get(tid, {}).get('_short', '') for tid in t_ids if tid]
                if t_names:
...
@bot.callback_query_handler(func=lambda call: call.data.startswith(('set_class_', 'set_teach_')))
...
def show_free_rooms(chat_id, message_id=None):
    ...
    busy_rooms = {c['_classroomids'] for c in cards if c['_days'][current_day] == '1' and c['_period'] == '1'}
    ...
    if message_id:
        bot.edit_message_text(text, chat_id, message_id, reply_markup=get_main_menu(), parse_mode="Markdown")
    else:
